...
Description: Extend your the userprofile - * PHP4+ or Function Library Plugin from my blog.
Author: Ralf Hortt
Version: 1.2.1
 ...
 * Edit the formular in the 'extented_user_profile' function
 * Activate it in WP Backend
 * To retrieve the meta datas simply call '<?php $meta = call_usermeta(); ?>'
*/

#add_action('register_form', extend_register_user_profile); // Remove "#" if you want that users can fill out the form on registration.
#add_action('register_post', extended_user_profile_registration_save); // Remove "#" if you want that users can fill out the form on registration.

add_action('show_user_profile',extend_user_profile); // Do Not Remove This Line
add_action('edit_user_profile',extend_user_profile); // Do Not Remove This Line
add_action('profile_update',extended_user_profile_save); // Do Not Remove This Line
...
global $wpdb;
	...
	<tr>
		...
		<td><input type="text" name="info" id="info" value="<?php echo $meta->info; ?>" /></td>
	</tr>

	<tr>
		<th><label for="info2">Textarea</label></th>
		<td><textarea name="info2" id="info2"><?php echo $meta->info2; ?></textarea></td>
	</tr>
	<?php // ---------- No more editing ?>
	</table>
	<?php
...
//======================================
// Description: The form to extend the profile
Function extend_register_user_profile() {
global $wpdb;
	...
	<p>
		...
		<input type="text" name="info" id="info" value="<?php echo $meta->info; ?>" style="width: 100%" />
	</p>

	<p>
		<label for="info2">Textarea</label><br />
		<textarea name="info2" id="info2"  style="width: 100%"><?php echo $meta->info2; ?></textarea>
	</p>
	<?php // ---------- No more editing
}
...
Function extended_user_profile_save(){
global $wpdb;

	// Filtering queries which were already sent by WP
	$ignore = extended_user_profile_ignore();
	
	// Update Meta data
	foreach($_POST as $key => $value) {
			if	(!in_array($key,$ignore)) {
					$value = strip_tags($value); // Remove HTML and PHP tags
					update_usermeta($_POST['user_id'] ,$key ,$value);
			}				
	}
}
...
Function extended_user_profile_registration_save() {
global $wpdb;
	$ignore = extended_user_profile_ignore();
...
// Note: Important if other Plugins where run on Userprofile or Registration
// Develop: Experimental, might throw it out as it's just needed for saving into DB
Function extended_user_profile_ignore($mode = "trash"){
	...
	$trash = array( '_wpnonce', '_wp_http_referer', 'wp_http_referer', 'wpnonce', 'from', 'checkuser_id', 'rich_editing', 'admin_color', 'first_name', 'last_name', 'nickname', 'display_name',
					'email', 'url', 'aim', 'yim', 'jabber', 'description', 'pass1', 'pass2', 'action', 'user_id', 'submit'); # TRASH USER PROFILE
